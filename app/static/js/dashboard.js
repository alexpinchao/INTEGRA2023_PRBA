...
function createChartExpansion(list_name, indicator, key, dataModel_pruebas_expansion, chart_title, line_chart, graph_container) {
    var row = key;
    var row_table = row;
    var al = Math.floor((Math.random() * (255)));
    // var al2 = Math.floor((Math.random() * (255)));
    var b_colorslist = ['rgba(' + al + ', 99, 132, 1)', 'rgba(' + al + ', 162, 235, 1)', 'rgba(' + al + ', 206, 86, 1)', 'rgba(' + al + ', 192, 192, 1)', 'rgba(' + al + ', 102, 255, 1)', 'rgba(' + al + ', 159, 64, 1)'];
    ...
    if (last_indicator_3 == indicator) {
        if (!(keys_list_3.includes(row_table))) {
            ...
            chart_element_3.data.datasets = chart_element_3.data.datasets.filter(data => data.label != row_table);
            chart_element_3.update();
        }
        ...
        let delayed;
        $('#' + line_chart).remove();
        $('#' + graph_container).append('<canvas id="' + line_chart + '" width="100%" height="48vh"><canvas>');
        var config = {
            type: 'bar',
            data: {
                labels: labels,
...
function createChartUpgrade(list_name, indicator, key, dataModel_pruebas_expansion, chart_title, line_chart, graph_container) {
    var row = key;
    var row_table = row;
    var al = Math.floor((Math.random() * (255)));
    // var al2 = Math.floor((Math.random() * (255)));
    ...
    if (last_indicator_4 == indicator) {
        if (!(keys_list_4.includes(row_table))) {
            ...
            chart_element_4.data.datasets = chart_element_4.data.datasets.filter(data => data.label != row_table);
            chart_element_4.update();
        }
...
function createChartIndicador1(list_name, indicator, key, dataModel_pruebas_expansion, chart_title, line_chart, graph_container) {
    var row = key;
    ...
    if (last_indicator_5 == indicator) {
        if (!(keys_list_5.includes(row_table))) {
            ...
            chart_element_5.data.datasets = chart_element_5.data.datasets.filter(data => data.label != row_table);
            chart_element_5.update();
        }
...
function createChartIndicador2(list_name, indicator, key, dataModel_pruebas_expansion, chart_title, line_chart, graph_container) {
    var row = key;
    ...
    if (last_indicator_6 == indicator) {
        if (!(keys_list_6.includes(row_table))) {
            ...
            chart_element_6.data.datasets = chart_element_6.data.datasets.filter(data => data.label != row_table);
            chart_element_6.update();
        }
...
function createChartIndicador3(list_name, indicator, key, dataModel_pruebas_expansion, chart_title, line_chart, graph_container) {
    var row = key;
    ...
    if (last_indicator_7 == indicator) {
        if (!(keys_list_7.includes(row_table))) {
            ...
            chart_element_7.data.datasets = chart_element_7.data.datasets.filter(data => data.label != row_table);
            chart_element_7.update();
        }
...
function generateGhapExpansion(sub_strategies,n,strategies_name){
    ...
    sub_strategies_key.forEach(function(item, index){
        ...
        let data_model_expansion = modelExpansionEstrategy(n, sub_strategies_value, sub_strategies_fp, sub_strategies_name);
...
function generateGhapUpgrade(sub_strategies,n,strategies_name){
    ...
    sub_strategies_key.forEach(function(item, index){
        ...
        let plot_upgrade = modelUpgradeStrategy(n, sub_strategies_value, sub_strategies_data, sub_strategies_n_LB, sub_strategies_name);
...
function generationIndicatorEficiency(generacion, consumo, name) {
    ...
    createChartIndicador1(list_name, "Indicador de eficiencia energética", name, data_plot_return, "chart_title_5", "line-chart-5", "graph-container-5");
...
function generationIndicatorsIEP(consumo, pib, name) {
    ...
    console.log("--data_plot_return--", data_plot_return);
    createChartIndicador2(list_name, "Indicador intensidad energética primaria", name, data_plot_return, "chart_title_6", "line-chart-6", "graph-container-6");
    return data_plot_return;
...
function generationIndicatorsIEC(consumo, fe, pib, name) {
    ...
    console.log("--data_plot_return--", data_plot_return);
    createChartIndicador3(list_name, "Indicador intensidad de emisiones de carbono", name, data_plot_return, "chart_title_7", "line-chart-7", "graph-container-7");
    return data_plot_return;
}
...
function generateDataIndicatorWithBau(sub_strategies_expansion,sub_strategies_upgrade,n){
    ...
    sub_strategies_key.forEach(function(item, index){
        ...
        let data_model_expansion1 = modelExpansionEstrategy(n, sub_strategies_expansion_value, sub_strategies_expansion_fp, sub_strategies_expansion_name);
    ...
    sub_strategies_upgrade_key.forEach(function(item, index){
        ...
        let data_model_upgrade1 = modelUpgradeStrategy(n, sub_strategies_value, sub_strategies_data, sub_strategies_n_LB, sub_strategies_name)
        //createChartUpgrade(list_name, strategies_name, sub_strategies_name, plot_upgrade, "chart_title_4", "line-chart-4","graph-container-4");
...
function createStrategyExpansionComplementary(sub_strategies,strategies_array_copia,n) {
    ...
    let ids_relatioated = sub_strategies_expansion
        .map((sub) => {
    ...
    let strategies_by_id_relationated = filterStrategiesByIdRelationated(strategies_array_copia_process, ids_relatioated)
    let sub_strategies_upgrade = strategies_by_id_relationated.models[1]
    console.log("entra a expansion need strategies_by_id_relationated", strategies_by_id_relationated.models[1])
    let strategies_complementaries = sub_strategies_upgrade.strategies
        .filter((strategy) => sub_strategies_expansion.some((strategy2) => strategy2.id_relation.includes(strategy.id_relation)))
    ...
    let create_data_indicator = generateDataIndicatorWithBau(sub_strategies_expansion, strategies_complementaries, n);
...
function createStrategyUpgradeComplementary(sub_strategies,strategies_array_copia,n) {
    ...
    let ids_relatioated = sub_strategies_upgrade
        .map((sub) => {
            let id_relationated = sub.id_relation
            return id_relationated
        })
    ...
    let strategies_array_copia_process = filterStrategiesByProcess(strategies_copia)
    let strategies_by_id_relationated = filterStrategiesByIdRelationated(strategies_array_copia_process, ids_relatioated)
    console.log("entra a actualizacion need strategies_by_id_relationated", strategies_by_id_relationated.models[0])
    ...
    let strategies_complementaries = sub_strategies_expansion.strategies
        .filter((strategy) => sub_strategies_upgrade.some(strategy2 => strategy2.id_relation.includes(strategy.id_relation)))
        .map((sub_strategies_upgrade_list) => {
            let newSubStrategies = Object.assign({}, sub_strategies_upgrade_list)
            return newSubStrategies
        })
    console.log("entra a expansion strategies_complementaries", strategies_complementaries)
    let create_data_indicator = generateDataIndicatorWithBau2(strategies_complementaries, sub_strategies_upgrade, n);
    let create_data_indicator_generation = create_data_indicator[0];
    console.log("create_data_indicator_generation", create_data_indicator_generation)
    let create_data_indicator_consume = create_data_indicator[1]
    console.log("create_data_indicator_consume", create_data_indicator_consume)
...
function createGraphIndicator(create_data_indicator_generation,create_data_indicator_consume){
    ...
    let pib = [344.1612833,	355.4685381,	367.8705797,	380.837696,	394.8857455,	408.9742253,	423.1216551,	437.1906672,	451.7446956];
    let fe = 0.13;
...
function plotDataStrategies(strategies){
    ...
    if (Object.entries(strategiesModels.filter((model) => strategiesName[0].includes(model.name))).length  !=0
        && Object.entries(strategiesModels.filter((model) => strategiesName[1].includes(model.name))).length  !=0 ){
        let a = strategiesModels[0]
        let b = strategiesModels[1]

        if (Object.entries( b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation.includes(strategy.id_relation)))).length != 0){
            //aqui grafica cuando tiene las dos estrategias y que estan relacionadas entre si
            let strategies_relationated = b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation.includes(strategy.id_relation))).map(
                (sub_strategies) => {
                    let newSubStrategies = Object.assign({},sub_strategies, {
                        values_BAU: a.strategies.filter(strategyValue => strategyValue.id_relation.includes(sub_strategies.id_relation)).map(
                            (sub_strategies_values) => {
                                let newSubStrategies =  modelExpansionEstrategyOnlyData(n, parseFloat(sub_strategies_values.selected_value), sub_strategies_values.fp)
                                return newSubStrategies
                            })[0]
                    })
                    return newSubStrategies
            })
            let sub_str_not_included = b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation.includes(strategy.id_relation))).map(
                (sub) => {let new_ids = sub.id_relation
                return new_ids}
            )
            ...
            if (Object.entries(b.strategies.filter(strategy => !sub_str_not_included.includes(strategy.id_relation))).length != 0){
                let sub_str_not_relationed = b.strategies.filter(strategy => !sub_str_not_included.includes(strategy.id_relation))
        ...
        }else if (Object.entries(b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation !== strategy.id_relation))).length != 0){
            //aqui grafica cuando tiene las dos estrategias pero esta no esta relacionada con las sub estrategias de expansion
            let sub_strategies_no_relationed =  b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation !== strategy.id_relation))
            generateGhapExpansion(a.strategies,n,strategiesName[0])
...
function plotDataIndicators(strategies){
    ...
    if (Object.entries(strategiesModels.filter((model) => strategiesName[0].includes(model.name))).length  !=0
        ...
        if (Object.entries( b.strategies.filter(strategy => a.strategies.some( strategy2 => strategy2.id_relation.includes(strategy.id_relation)))).length != 0){
            ...
            let sub_str_not_included_expansion = a.strategies.filter(strategy => b.strategies.some( strategy2 => strategy2.id_relation.includes(strategy.id_relation))).map(
                (sub) => {let new_ids = sub.id_relation
                return new_ids}
            )
            ...
            let create_data_indicator = generateDataIndicatorWithValues(a.strategies,strategies_relationated,n);
            let create_data_indicator_generation = create_data_indicator[0];
            let create_data_indicator_consume = create_data_indicator[1]
            createGraphIndicator(create_data_indicator_generation,create_data_indicator_consume)
            ...
            if (Object.entries(b.strategies.filter(strategy => !sub_str_not_included.includes(strategy.id_relation))).length != 0){
                ...
                //generateGhapUpgrade(sub_str_not_relationed,n,strategiesName[1])
                console.log("como es sub_str_not_relationed no incluye actualizacion",sub_str_not_relationed)
            }
            if(Object.entries(a.strategies.filter(strategy => !sub_str_not_included_expansion.includes(strategy.id_relation))).length != 0){
                let sub_str_not_relationed = a.strategies.filter(strategy => !sub_str_not_included_expansion.includes(strategy.id_relation))
